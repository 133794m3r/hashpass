...
function simplify(a,d,b){var c=Date.now(),e=RegExp("[^a-z]","g"),f=a.replace(RegExp("[^0-9]","g"),""),g,h,l,k="";l=(a.charCodeAt(1)+a.charCodeAt(1)+a.charCodeAt(2))%5;a=a.replace(e,"");console.log(a);var m,e=a.substr(1);g=e=no_repeat_strings(e);e=a.substr(0,1).toUpperCase();h=f.substr(0,1);f=f.substr(1);f=no_repeat_strings(f);m=parseInt(f.substr(0,2))%3;if(!1===b)switch(m){case 0:k="$";break;case 1:k="#";break;case 2:k="@"}else k=5<=f.length?f.substr(-1,1):g.substr(-1,1);5<=f.length?(num_str_len=
Math.floor(d/2),g=g.substr(0,num_str_len-2),f=f.substr(0,d-num_str_len-1)):(d-=f.length,g=g.substr(0,d-3),f.substr(0));switch(l){case 0:a=h+e+g+f+k;break;case 1:a=e+h+g+f+k;break;case 2:a=h+e+f+g+k;break;case 3:a=f+e+g+h+k;break;case 4:a=f+e+h+g+k}d=Date.now();console.log("simplify:"+(d-c)+"ms");return a}
...
function xor_rand(a,d){var b,c,e,f,g;b=window.screenX^window.screen.width;c=window.screenY^window.screen.height;e=Math.floor(4294967295*Math.rand());f=Math.floor(4294967295*Math.rand());f=(f^f<<1)>>3;f^=f<<10;b^=b<<16;e=b>>5^b<<1;c=(c^c>>2)<<5;c^=c>>15;var h=(f&6)+(b&6)+(c&6)+(e&6)+3,l;for(l=0;l<h;++l)f^=f<<1,f>>=3,f^=f<<10,e^=e<<1,b^=b<<16,e=b>>5,e^=b<<1,c^=c>>2,c<<=5,c^=c>>15;h*=2;for(l=0;l<h;++l)g=b^b<<20^c^c>>11^e^e<<27^f^f>>6,b=c,c=e,e=f,f=g;return a||d?Math.floor(f*(d-a))+a:f&4294967295}
function microtime(){return Date.now()/1E3};function base32_encode(a){var d="",b=Math.floor(a.length/5),c=a.length%5,e,f,g,h,l,k,m,n="";0!=c&&(b+=1);for(i=0;i<b;i++)f=a.charCodeAt(5*i),h=a.charCodeAt(5*i+1),l=a.charCodeAt(5*i+2),k=a.charCodeAt(5*i+3),m=a.charCodeAt(5*i+4),e=f>>3,f=(f&7)<<2|h>>6,g=(h&63)>>1,h=(h&1)<<4|l>>4,l=(l&15)<<1|k>>7,parts6=(k&127)>>2,k=(k&3)<<3|m>>5,m&=31,d+="ybndrfg8ejkmcpqxot1uwisza345h769".charAt(e)+"ybndrfg8ejkmcpqxot1uwisza345h769".charAt(f)+"ybndrfg8ejkmcpqxot1uwisza345h769".charAt(g)+"ybndrfg8ejkmcpqxot1uwisza345h769".charAt(h)+
...
function no_repeat_strings(a){for(var d="",b=a.length,c="",e=a.substr(0,1),f,g=1,h=1,l=0,k=0,m=0;1===g;)h>b&&1<=l&&100>=k?(m=m+h+k,h=1,a=d+c,a.substr(-1,1)===a.substr(-2,1)?(ins_place=m%b,a=a.substr(0,ins_place)+a.substr(-1,1)+a.substr(ins_place,b-1)):a.substr(0,1)===a.substr(1,1)&&(ins_place=m%b,a=a.substr(0,ins_place)+a.substr(-1,1)+a.substr(ins_place,b-1)),e=a.substr(0,1),l=0,g=1,d=c="",k+=1):h>b&&(g=0),f=a.substr(h,1),e==f?(d+=e,l=d.length):c+=e,e=f,h+=1;return a};
