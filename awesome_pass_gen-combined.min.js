...
*/
"use strict"
var perc=0;function generate_salt(a,b,d,e,c){e=void 0===e?!1:e;c=void 0===c?!1:c;var g=Date.now(),f=7,h=6,k=9,m=8;1==e&&!1===c?(a=scrypt(a,d,f,h,1,32,"base64"),b=scrypt(b,a,f,h,1,32,"hex"),d=scrypt(d,b,f,h,1,32,"base64")):(!0===c&&(f=h=9,k=13,m=15),h+=1,m+=1,a=ucrypt(a,d,f,h,1,32,"base64"),b=ucrypt(b,a,f,h,1,32,"hex"),d=ucrypt(d,b,f,h,1,32,"base64"));c=d+a+b;a=b+d+a;c=!0===e?scrypt(c,a,k,m,1,32,"hex"):ucrypt(c,a,k,m,1,32,"hex");e=Date.now();console.log("gen_salt:"+(e-g)+"ms");return c}
...
function generate_pass(a){a=void 0===a?!1:a;document.getElementsByTagName("input");var b=[],d=select_by_id("no_spec").checked,e=select_by_id("no_legacy").checked;var c=select_by_id("no_security").checked;var g=select_by_id("site_name").value;var f=select_by_id("password").value;var h=select_by_id("username").value;var k=select_by_id("length").value;b[0]=h;b[1]=g;""===k&&(k=14,select_by_id("length").value=14);if(""!==f){var m=f.length;f=f.substr(0,1).toUpperCase()+f.substr(1,m);select_by_id("password").value=
f}""!==h&&(h=h.substr(0,1).toUpperCase()+h.substr(1),select_by_id("username").value=h);""!==g&&(g=g.substr(0,1).toUpperCase()+g.substr(1),select_by_id("site_name").value=g);var l=zxcvbn(f,b);m=l.feedback.warning;var q="Try adding a word or two, less common words are better. Or try adding a few numbers. "+m;var n=l.score;if(0===n){var p="red";var r="Unsafe";document.getElementById("orig_score_txt").setAttribute("style","color: red; font-weight:bold;")}else 1===n?(p="orange",r="Bad"):2===n?(p="yellow",
r="Acceptable"):3===n?(p="yellowgreen",r="Safe"):(p="green",r="Perfect");select_by_id("orig_score_txt").setAttribute("style","color: white; font-weight:bold;");select_by_id("orig_score_txt").innerHTML=n;var t=23.75*n;select_by_id("low_score_warn").setAttribute("style","margin-left:29%; background-color:black; color:"+p);select_by_id("low_score_warn").innerHTML=r;select_by_id("orig_score_bar").setAttribute("style","width:"+t+"%; background-color:"+p);1>=l.score&&(b[0]===b[1]||b[0]===f||b[1]===f)?select_by_id("feedback").innerHTML=
"Do not use your username or site name in the password! "+q:2>=l.score?select_by_id("feedback").innerHTML=q:select_by_id("feedback").innerHTML="Score is 3 or above and thus suggestions not necessary";b=Date.now();n=generate_salt(f,h,g,e);h=Date.now();!0===e?(select_by_id("orig_time").innerHTML=display_time(l.guesses/1300),f=scrypt(f,n,16,6,2,32,"binary")):!1===c?(select_by_id("orig_time").innerHTML=display_time(l.guesses/1300),f=ucrypt(f,n,16,12,1,32,"binary"),select_by_id("length").value=14):(select_by_id("orig_time").innerHTML=
display_time(l.guesses/1300),k++,f=ucrypt(f,n,17,16,1,32,"binary"));c=Date.now();f=base32_encode(f,!1);f=simplify(f,k,d,e);select_by_id("result").value=f;k=Date.now();d=k-b;b=Date.now();l=zxcvbn(f);k=Date.now();n=l.score;p="green";t=23.75*n;select_by_id("gen_score_txt").innerHTML=n;select_by_id("gen_score_bar").setAttribute("style","width:"+t+"%; background-color:"+p);select_by_id("gen_time").innerHTML=display_time(l.guesses/9E3);modal_toggle("_progress");select_by_id("generate_pass").disabled=!1;
!0===a&&(console.log(l.guesses),console.log(m),alert("total new values:"+d),console.log("scrypt time:"+(c-h)+"ms"),console.log("total:"+(k-b)+"ms"),console.log("b "+n),console.log("zxcvbn:"+(k-b)+"ms"));return 0}function percent_update(a){var b=select_by_id("completed_perc"),d=select_by_id("perc_text"),e=select_by_id("perc_done");a+="%";d.innerHTML=a;e.innerHTML=a;b.style.width=a}
...
"width:"+h+"%; background-color:"+f);select_by_id("low_score_warn").setAttribute("style","margin-left:29%;color:"+f);select_by_id("low_score_warn").innerHTML=c;1>=e.score&&(d==b||a==b)?select_by_id("feedback").innerHTML="Do not use your username or site name in the password! "+g:2>=e.score?(console.log("hit"),select_by_id("feedback").innerHTML=g):select_by_id("feedback").innerHTML="Score is 3 or above and thus suggestions not necessary";sselect_by_id("orig_time").innerHTML=display_time(e.guesses/
...
function microtime(){return Date.now()/1E3}function display_time(a){var b;a=1>a?[null,"less than a second"]:60>a?(b=Math.round(a),[b,b+" second"]):3600>a?(b=Math.round(a/60),[b,b+" minute"]):86400>a?(b=Math.round(a/3600),[b,b+" hour"]):2678400>a?(b=Math.round(a/86400),[b,b+" day"]):32140800>a?(b=Math.round(a/2678400),[b,b+" month"]):321408E4>a?(b=Math.round(a/32140800),[b,b+" year"]):[null,"centuries"];b=a[0];a=a[1];null!=b&&1!==b&&(a+="s");return a};function base32_encode_old(a){var b="",d=Math.floor(a.length/5),e=a.length%5,c,g="";0!=e&&(d+=1);for(c=0;c<d;c++){var f=a.charCodeAt(5*c);var h=a.charCodeAt(5*c+1);var k=a.charCodeAt(5*c+2);var m=a.charCodeAt(5*c+3);var l=a.charCodeAt(5*c+4);var q=f>>3;f=(f&7)<<2|h>>6;var n=(h&63)>>1;h=(h&1)<<4|k>>4;k=(k&15)<<1|m>>7;var p=(m&127)>>2;m=(m&3)<<3|l>>5;l&=31;b+="ybndrfg8ejkmcpqxot1uwisza345h769".charAt(q)+"ybndrfg8ejkmcpqxot1uwisza345h769".charAt(f)+"ybndrfg8ejkmcpqxot1uwisza345h769".charAt(n)+"ybndrfg8ejkmcpqxot1uwisza345h769".charAt(h)+
"ybndrfg8ejkmcpqxot1uwisza345h769".charAt(k)+"ybndrfg8ejkmcpqxot1uwisza345h769".charAt(p)+"ybndrfg8ejkmcpqxot1uwisza345h769".charAt(m)+"ybndrfg8ejkmcpqxot1uwisza345h769".charAt(l)}a=0;1==e?a=6:2==e?a=4:3==e?a=3:4==e&&(a=1);e=8*c;for(c=0;c<a;c++)g+="=";return b=b.substr(0,e-a)+g}
function base32_encode(a,b){var d="",e=Math.floor(a.length/5),c=a.length%5,g="";0!=c&&(e+=1);if(!0===(void 0===b?!0:b))for(b=0;b<e;b++){var f=5*b;var h=a.charCodeAt(f);var k=a.charCodeAt(f+1);var m=a.charCodeAt(f+2);var l=a.charCodeAt(f+3);var q=a.charCodeAt(f+4);f=h>>3;h=(h&7)<<2|k>>6;var n=(k&63)>>1;k=(k&1)<<4|m>>4;m=(m&15)<<1|l>>7;var p=(l&127)>>2;l=(l&3)<<3|q>>5;q&=31;d+="ybndrfg8ejkmcpqxot1uwisza345h769".charAt(f)+"ybndrfg8ejkmcpqxot1uwisza345h769".charAt(h)+"ybndrfg8ejkmcpqxot1uwisza345h769".charAt(n)+
"ybndrfg8ejkmcpqxot1uwisza345h769".charAt(k)+"ybndrfg8ejkmcpqxot1uwisza345h769".charAt(m)+"ybndrfg8ejkmcpqxot1uwisza345h769".charAt(p)+"ybndrfg8ejkmcpqxot1uwisza345h769".charAt(l)+"ybndrfg8ejkmcpqxot1uwisza345h769".charAt(q)}else for(b=0;b<e;b++)f=5*b,h=a[f],k=a[f+1],m=a[f+2],l=a[f+3],q=a[f+4],f=h>>3,h=(h&7)<<2|k>>6,n=(k&63)>>1,k=(k&1)<<4|m>>4,m=(m&15)<<1|l>>7,p=(l&127)>>2,l=(l&3)<<3|q>>5,q&=31,d+="ybndrfg8ejkmcpqxot1uwisza345h769".charAt(f)+"ybndrfg8ejkmcpqxot1uwisza345h769".charAt(h)+"ybndrfg8ejkmcpqxot1uwisza345h769".charAt(n)+
"ybndrfg8ejkmcpqxot1uwisza345h769".charAt(k)+"ybndrfg8ejkmcpqxot1uwisza345h769".charAt(m)+"ybndrfg8ejkmcpqxot1uwisza345h769".charAt(p)+"ybndrfg8ejkmcpqxot1uwisza345h769".charAt(l)+"ybndrfg8ejkmcpqxot1uwisza345h769".charAt(q);a=0;1==c?a=6:2==c?a=4:3==c?a=3:4==c&&(a=1);c=8*b;for(b=0;b<a;b++)g+="=";return d=d.substr(0,c-a)+g}function hex_decode(a){var b=Math.ceil(a.length/2),d,e="";for(d=0;d<b;d++)e+=String.fromCharCode(parseInt(a.substr(2*d,2),16));return e}
